...
var DataTable = {
	 ...
	 *  FIXME: correct str() and "if value" references
	 ...
	 */
	singleValueToJS: function(value, type, escapeFn) {
		if( arguments.length < 3 ) { escapeFn = this._escapeValue; }
		var _t = this._t;
		...
		if( _t.isArray(value) ) {
			...
			js_value = this.singleValueToJS(value[0], type);
			if( value[1] == null ) {
	 ...
	 */
	columnTypeParser: function(description) {
		var _t = this._t;
	 ...
	 */
	tableDescriptionParser: function(tableDescription, depth) {
		...
		if( _o.prop(tableDescription).length != 1 ||
			...
			columns = [];
			// FIXME: the properties are sorted at this point
			for( var i in tableDescription ) {
				...
				if( _t.isArray(tableDescription[i]) ) {
					...
					parsedCol = this.columnTypeParser(tableDescription[i]);
				} else {
					parsedCol = this.columnTypeParser([i, tableDescription[i]]);
				}
				parsedCol.depth = depth;
		...
		var parsedCol = this.columnTypeParser(_o.prop(tableDescription)[0]);
		parsedCol.depth = depth;
		parsedCol.container = 'dict';

		var result = this.tableDescriptionParser(_o.val(tableDescription)[0],depth + 1)
	...
	_escapeValue: function(v) {
		// FIXME: This is incorrect. I'm not certain what does and doesn't need escaped
	...
	_escapeCustomProperties: function(properties) {
		l = [];
		for( var key in properties ) {
			l.push(this._escapeValue(key)+':'+this._escapeValue(properties[key]));
		}
		return '{'+l.join(',')+'}';
	},
